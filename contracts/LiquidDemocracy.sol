...
// TODO: Decide whether to make Delegation a separate app
contract LiquidDemocracy is Delegation {
  ...
  * @param _staking Address of the staking app where voters need to be staked to vote
  ...
  */
  ...
  function canExecute(uint256 _voteId) public view returns (bool) {
    ...
    /*
    // Votes cannot be prematurely executed even if decided to give a chance for overrules
  ...
  function _newVote(bytes _executionScript, string _metadata, bool _castVote) isInitialized internal returns (uint256 voteId) {
    ...
    // TODO: should probably use the total token supply rather than the total staked
    vote.totalVoters = totalStakedAt(vote.snapshotBlock);
  ...
  ) internal
  {
      ...
      uint256 voterStake = powerAt(_voter, vote.snapshotBlock);
      VoterState previousVote = vote.voters[_voter];
      
      if (voterStake > 0) {
        // Voter hasn't delegated and is voting on their behalf and their delegates
        voterStake = voterStake.sub(vote.overruledVotes[_voter]);
      } else if (vote.voters[_voter] == VoterState.Absent){
        // If voter has 0 direct power, they are either overruling their delegate or don't have voting power
        uint256 directStake = totalDelegatedBalanceAt(_voter, vote.snapshotBlock);
        require(directStake > 0);

        address[] memory chain = delegationChainAt(_voter, vote.snapshotBlock);

        // Votes overruled to voter have already been propagated up the chain
        uint256 overrulingVotes = directStake.sub(vote.overruledVotes[_voter]);

        voterStake = overrulingVotes;

        for (uint256 i = 0; i < chain.length; i++) {
          address delegate = chain[i];
          vote.overruledVotes[delegate] = vote.overruledVotes[delegate].add(overrulingVotes);

          if (vote.voters[delegate] != VoterState.Absent) {
            previousVote = vote.voters[delegate];
            break; // No need to continue overruling up the chain, as it has already been overruled
          }
        }
      } else {
        // Already overruled, but changing vote
        uint256 overruled = vote.overruledVotes[_voter];
        voterStake = totalDelegatedBalanceAt(_voter, vote.snapshotBlock).sub(overruled);
      }

      // if voter had previously voted, decrease count
      if (previousVote == VoterState.Yea)
          vote.yea = vote.yea.sub(voterStake);
      if (previousVote == VoterState.Nay)
